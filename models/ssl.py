...
from utils_ssl import datautils, utils
from utils_ssl.lars_optimizer import LARS
...
class Final_MobiusTransform_SSL_CPU(nn.Module):
    
    def __init__(self, a_real=1.0, a_imag=0.0, b_real=0.0, b_imag=0., c_real=.20, c_imag=0.0, d_real=.75, d_imag=0., p=0.8):
        print(f'Adding MobiusTransform CPU in augmentation pipeline with probability {p}')
        super(Final_MobiusTransform_SSL_CPU, self).__init__()

        self.a = torch.tensor(complex(a_real, a_imag))
        self.b = torch.tensor(complex(b_real, b_imag))
        self.d = torch.tensor(complex(d_real, d_imag))
        self.p = p
    ...
    def _randomize_c(self):
        ...
        return torch.tensor(complex(c_real, c_imag))
    ...
    def forward(self, image):
        with torch.no_grad():
            ...
            if decision:
                ...
                z_transformed = z.mul_(self.a).add_(self.b).div_(c.mul(z).add_(self.d))

                x_transformed = torch.real(z_transformed)
                y_transformed = torch.imag(z_transformed)
...
class Final_MobiusTransform_SSL_NB_CPU(nn.Module):
    
    def __init__(self, a_real=1.0, a_imag=0.0, b_real=0.0, b_imag=0., c_real=.20, c_imag=0.0, d_real=.75, d_imag=0., p=0.8):
        print(f'Adding MobiusTransform CPU with background interpolation in augmentation pipeline with probability {p}')
        super(Final_MobiusTransform_SSL_NB_CPU, self).__init__()
    ...
    def forward(self, image):
        with torch.no_grad():
            ...
            if decision:
                ...
                # Convert coordinates from [-1, 1] to [0, width-1] or [0, height-1] 
                x_transformed = (x_transformed + 1) * (width // 2)
                y_transformed = (y_transformed + 1) * (height // 2)
...
class BaseSSL(nn.Module):
    ...
    IMAGENET_PATH = os.environ.get('IMAGENET_PATH', '/home/prachh/datasets/imagenet_1k_2012/ILSVRC/Data/CLS-LOC/')
    ...
    def load(cls, ckpt, device=None):
        ...
        print('hparams before', hparams.dist)
        res = cls(hparams, device=device)
        print('hparams after', hparams.dist)
    ...
    def prepare_data(self):
        ...
        if self.hparams.data == 'cifar':
            ...
            valdir = os.path.join(self.IMAGENET_PATH, self.hparams.val_portion)
...
class SimCLR(BaseSSL):
    ...
    def transforms(self):
        if self.hparams.data == 'cifar':
            ...
            from utils_ssl.datautils import GaussianBlur

            im_size = 224
            print('self.hparams.mobius_prob ', self.hparams.mobius_prob)
            if False == self.hparams.mobius_background:
                train_transform = transforms.Compose([
                    Final_MobiusTransform_SSL_CPU(p=self.hparams.mobius_prob),
                    transforms.RandomResizedCrop(
                        im_size,
                        scale=(self.hparams.scale_lower, 1.0),
                        interpolation=PIL.Image.BICUBIC,
                ...
                train_transform = transforms.Compose([
                Final_MobiusTransform_SSL_NB_CPU(p=self.hparams.mobius_prob),
                transforms.RandomResizedCrop(
                    im_size,
                    scale=(self.hparams.scale_lower, 1.0),
                    interpolation=PIL.Image.BICUBIC,
...
class SSLEval(BaseSSL):
    ...
    def __init__(self, hparams, device=None):
        ...
        print(f'Finetuned set to {hparams.finetune}')
        ...
        if hparams.data == 'cifar':
            hdim = self.encode(x=torch.ones(10, 3, 32, 32).to(device)).shape[1]
            n_classes = 10
        elif hparams.data == 'imagenet':
            hdim = self.encode(x=torch.ones(10, 3, 224, 224).to(device)).shape[1]
            #print('hdim ', hdim)
            n_classes = 1000
    ...
    def transforms(self):
        if self.hparams.data == 'cifar':
            ...
            if False == self.hparams.mobius_background:
                train_transform = transforms.Compose([
                    Final_MobiusTransform_SSL_CPU(p=self.hparams.mobius_prob),
                    #Final_MobiusTransform_SSL(p=self.hparams.mobius_prob, device = 0),
                    transforms.RandomResizedCrop(
                        224,
                        scale=(self.hparams.scale_lower, 1.0),
                        interpolation=PIL.Image.BICUBIC,
...
def configure_optimizers(args, model, cur_iter=-1, LARS_optimizer_weights = None):
    iters = args.iters
    #print('LARS_optimizer_weights ', cur_iter,  LARS_optimizer_weights.keys())
    ...
    if args.opt == 'sgd':
        ...
        # Workaround for creating initial_lr for each group for correctly resuming LR schedular
        if LARS_optimizer_weights != None:
            larc_optimizer.load_state_dict(LARS_optimizer_weights)
            for group in larc_optimizer.param_groups:
    ...
    if args.lr_schedule == 'warmup-anneal':
        ...
        if LARS_optimizer_weights != None:
            optimizer.load_state_dict(LARS_optimizer_weights)
            for group in optimizer.param_groups:
                group.setdefault('initial_lr', group['lr'])
        for group in optimizer.param_groups:
            group.setdefault('initial_lr', group['lr'])
